...
```{r echo = FALSE, out.width = "1.03\\textwidth", fig.align = "left"}
knitr::include_graphics("ext_tracks_3.png")
```
...
```{r echo = FALSE, out.width = "1.05\\textwidth", fig.align = "left"}
knitr::include_graphics("wind_model_eval_sandy_2012.png")
```
...
```{r eval = FALSE}
library(coin)
spearman_test(Distance ~ Rain, data = storm_severity)
spearman_test(Distance ~ Wind, data = storm_severity)
spearman_test(Distance ~ Flood, data = storm_severity)
spearman_test(Distance ~ Tornado, data = storm_severity)
```
